          ...
          <div class="detail-data-right">
            ...
            @foreach ($course->publishedLessons->take(3) as $lesson)
              <li>
                ...
                @else   
                  @if (!$purchased_course)
                    <a class="lesson-title" aria-disabled="false" style="cursor:  alias" href="#"><i class='bx bx-lock'></i>Drugi kurs {{ $lesson->count() }}</a>
                  @else  
                    <a class="lesson-title" href="{{ route('lessons.show', [$lesson->course_id, $lesson->slug]) }}"><i class="bx bx-play-circle"></i>{{ $lesson->title }}</a>
                  @endif
                @endif 
            ...
            @else
                ...
                class="button detail-button" style="text-align: center;">Kupite kurs (${{ $course->price }})</a>
            @endif
          </div>
        </div>
      </section>
